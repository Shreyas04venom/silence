...
let keyIndex = 0
function nextKey() {
  const k = keys[keyIndex % Math.max(keys.length, 1)]
  keyIndex++
  return k
}

async function generateSVGWithGemini(topic: string): Promise<string | null> {
    ...
    for (const modelName of modelsToTry) {
      try {
        ...
        const result = await model.generateContent(
          `Generate a detailed educational SVG diagram about "${topic}" for deaf/hard-of-hearing school students (Grade 6–8 level).

REQUIREMENTS:
- Pure SVG only — start with <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 750" width="1000" height="750">
- Include: background rect, bold title at top, labeled diagram sections with numbers
- Show the actual concept visually: use circles, rectangles, arrows (markers), paths
- Color-code sections (different fill colors per element)
- Add clear text labels for every element
- Use arrows to show flow/direction where applicable
- Scientific accuracy required
- No JavaScript, no animations, no external resources
- End with </svg>
...
Return ONLY the raw SVG markup, nothing else.`,
        ...
        if (svgMatch) {
          ...
          // Validate it has real drawn shapes
          if (
            svg.includes("<rect") ||
            svg.includes("<circle") ||
            svg.includes("<path") ||
            ...
            console.log(`[Image Generation] Gemini SVG generated for: ${topic}`)
            return svg
          }
        }
      } catch (e: unknown) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    // 1. Try hardcoded specialized SVG first (fast, reliable)
    const normalizedTopic = topic.toLowerCase()
    let svg: string | null = null

    if (normalizedTopic.includes("digestive") || normalizedTopic.includes("digestion")) {
      svg = generateDigestiveSystemDiagram()
    } else if (normalizedTopic.includes("photosynthesis") || normalizedTopic.includes("nutrition in plants")) {
      svg = generatePhotosynthesisDiagram()
    } else if (normalizedTopic.includes("cell") || normalizedTopic.includes("cell structure")) {
      svg = generateCellDiagram()
    } else if (normalizedTopic.includes("water cycle") || normalizedTopic.includes("water")) {
      svg = generateWaterCycleDiagram()
    } else if (normalizedTopic.includes("nervous") || normalizedTopic.includes("brain")) {
      svg = generateNervousSystemDiagram()
    } else if (normalizedTopic.includes("heart") || normalizedTopic.includes("circulatory")) {
      svg = generateCirculatorySystemDiagram()
    } else if (normalizedTopic.includes("respiration") || normalizedTopic.includes("lung")) {
      svg = generateRespiratorySystemDiagram()
    }

    // 2. If not a known topic, use Gemini to generate a topic-specific SVG
    if (!svg) {
      console.log(`[Image Generation] No hardcoded SVG for "${topic}" — calling Gemini…`)
